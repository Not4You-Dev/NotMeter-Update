(() => {
  ...
  const COPY = {
    ko: {
      ...
      classPerformanceDescription: "이번 주 검증 기록을 같은 보스·10K CP 조건으로 맞춘 상대 지표입니다. 원시 DPS 순위가 아닙니다.",
      classPerformanceCpTitle: "검증 기록만 수집",
      classPerformanceCpText: "이번 주 통계 검증을 통과한 보스 처치 중 서버·캐릭터·직업이 확인되고 CP가 400K~2M인 기록만 사용합니다.",
      ...
      classPerformanceNoticeText: "CP·보스·중복 캐릭터·표본 부족의 영향을 줄인 이번 주 실전 비교 지표입니다. A가 가장 안정적이고 C는 참고 가능한 최소 기준입니다. 파티 버프·보스별 역할·숙련도 차이까지 완전히 제거할 수는 없으므로 밸런스의 절대 판정이나 이론상 최대 DPS가 아니라, 실제 수집 기록에서 반복 관측된 경향으로 해석해 주세요.",
    ...
    en: {
      ...
      classPerformanceDescription: "A relative index built from this week's verified runs after matching the same boss and 10K CP bracket. It is not a raw-DPS leaderboard.",
      classPerformanceCpTitle: "Verified runs only",
      classPerformanceCpText: "Only accepted boss kills with a known server, character, and class, and record-time CP from 400K to under 2M, are used.",
      ...
      classPerformanceNoticeText: "This weekly real-world comparison reduces CP, boss, duplicate-character, and small-sample effects. A is the most stable grade, while C is the minimum useful reference. Party buffs, encounter roles, and player skill cannot be removed completely, so this is a repeatedly observed trend rather than an absolute balance verdict or theoretical maximum DPS.",
