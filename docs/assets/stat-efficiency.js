(() => {
  ...
  const TEXT = {
    ko: {
      ...
      calculate: "효율 계산", calculating: "계산 중…", resultKicker: "직업·스킬별 분석", resultTitle: "스탯 효율 순위", displayedStatsTitle: "게임 표시 스탯 검산", displayedStatsHelp: "입력 원본을 게임과 같은 방식으로 합산한 값입니다.", displayedAttack: "최종 공격력", displayedAccuracy: "최종 명중", displayedCritical: "최종 치명타", displayedDefense: "최종 방어력", waitingTitle: "표본을 수집하고 있습니다", waitingDescription: "모든 지원 보스와 선택한 직업·스킬이 높은 신뢰 기준을 통과하면 결과가 열립니다.", resultGuide: "+1, +10, +1%p 단위별 순위이며 서로 다른 단위끼리는 직접 비교하지 않습니다.", growthGroup: "주신 스탯 · +1 기준", integerGroup: "전투 수치 · +10 기준", percentGroup: "퍼센트 스탯 · +1%p 기준", easyTitle: "간편하게 사용하는 방법", easyOne: "딜미터기 처치 기록 상단에서 ‘내 스탯 복사’를 누릅니다.", easyTwo: "이 페이지 상단 입력칸에 붙여넣습니다.", easyThree: "대표 스킬·공격 방향을 선택하면 지원 던전 6개 보스를 동일 가중치로 종합 계산합니다.",
  ...
  function showCollecting(result) {
	renderDisplayedStats(result?.displayedStats);
  ...
  function renderDisplayedStats(stats) {
	const panel = document.getElementById("displayed-stats");
	if (!stats || !["attack", "accuracy", "critical", "defense"].every(key =>
		Number.isSafeInteger(Number(stats[key])) && Number(stats[key]) >= 0)) {
		panel.hidden = true;
		return;
	}
	for (const key of ["attack", "accuracy", "critical", "defense"]) {
		document.getElementById(`displayed-${key}`).textContent = formatCount(stats[key]);
	}
	panel.hidden = false;
  }

  async function previewDisplayedStats() {
	...
	try {
		...
		// The explicit Calculate action reports connectivity errors to the user.
	}
  }
