(() => {
  ...
  const PROFILE_FIELDS = [
	...
	const HIDDEN_EFFECTS = new Set(["allElementDamageAmplification"]);
  ...
  const TEXT = {
    ko: {
      ...
      methodAria: "스탯 효율 계산 기준", methodTitle: "어떻게 계산하나요?", methodDataTitle: "1. 실전 표본 수집", methodDataDescription: "지원 던전에서 최종 스탯·직업·스킬·공격 방향·보스 상태가 함께 확인된 피해만 사용합니다.", methodModelTitle: "2. 직업별 피해 모델", methodModelDescription: "여러 사용자의 표본을 교차 검증하고 특정 설치의 과도한 업로드 비중을 제한해 직업 전체 전투를 구성합니다.", methodCompareTitle: "3. 상승 전후 재계산", methodCompareDescription: "현재 기준에서 각 스탯을 +1, +10 또는 +1%p 바꿔 보스 기대 피해 변화량을 동일 조건으로 비교합니다.", methodValidationTitle: "4. 검증 후 공개", methodValidationDescription: "서로 다른 사용자 수와 중앙·상위 오차 기준을 통과한 직업 결과만 표시하며 실패 모델은 기존 결과를 대체하지 않습니다.",
      ...
      calculate: "내 스탯으로 다시 계산", calculating: "계산 중…", resultKicker: "직업 전체 분석", resultTitle: "스탯 효율 순위", displayedStatsTitle: "실시간 스탯 시뮬레이션", displayedStatsHelp: "복사 기준에서 입력값을 바꾸면 게임 표시 최종 스탯이 바로 갱신됩니다.", displayedAttack: "최종 공격력", displayedAccuracy: "최종 명중", displayedCritical: "최종 치명타", displayedDefense: "최종 방어력", displayedCooldownTime: "재시전 시간", displayedCombatSpeed: "전투 속도", simulationReset: "복사 기준으로 초기화", simulationDamage: "예상 종합 피해 변화", simulationWaiting: "기준 스탯을 붙여넣으면 자동 계산됩니다.", simulationCalculating: "변경값을 계산하고 있습니다.", simulationCollecting: "선택한 직업은 아직 검증 표본을 수집 중이며 최종 스탯은 정상 계산되었습니다.", simulationReady: "직업 전체 전투와 검증 완료 보스를 종합 계산한 결과입니다.", waitingTitle: "선택한 직업의 표본을 검증하고 있습니다", waitingDescription: "선택한 직업과 지원 보스 표본이 공개 가능한 검증 기준을 통과하면 결과가 열립니다.", resultGuide: "+1, +10, +1%p 단위별 순위이며 서로 다른 단위끼리는 직접 비교하지 않습니다.", overviewSource: "직업 대표 표본 기준", personalSource: "내 캐릭터 스탯 기준", growthGroup: "주신 스탯 · +1 기준", attackGroup: "공격 수치 · +10 기준", judgmentGroup: "판정 수치 · +10 기준", percentGroup: "퍼센트 스탯 · +1%p 기준", directionalGroup: "공격 방향별 효율 · +1%p 기준", directionalHelp: "전방·후방의 1%p 계산식은 같습니다. 효율 차이는 직업별 실제 방향 피해 비중과 현재 스탯에서 발생합니다.", frontCondition: "전방 타격에서만 적용", backCondition: "후방 타격에서만 적용", observedDamageShare: "실전 피해 비중 {share}%", detailRanking: "전체 효율 순위 · {count}개 항목", easyTitle: "간편하게 사용하는 방법", easyOne: "직업을 선택하면 스탯 입력 없이 전체 표본 통계가 바로 표시됩니다.", easyTwo: "내 캐릭터에 맞추려면 딜미터기에서 ‘내 스탯 복사’ 후 붙여넣습니다.", easyThree: "붙여넣은 스탯은 자동 계산되며 입력값을 바꾸면 실시간으로 다시 보정됩니다.",
    ...
    en: {
      ...
      methodAria: "Stat efficiency calculation method", methodTitle: "How is it calculated?", methodDataTitle: "1. Collect combat samples", methodDataDescription: "Only damage with confirmed final stats, class, skill, attack direction, and boss state from supported dungeons is used.", methodModelTitle: "2. Build a class model", methodModelDescription: "Samples from different users are cross-validated and any single installation's upload weight is capped.", methodCompareTitle: "3. Recalculate each increase", methodCompareDescription: "Each stat is changed by +1, +10, or +1%p and the expected boss damage change is compared under the same conditions.", methodValidationTitle: "4. Publish after validation", methodValidationDescription: "Only class results that pass distinct-user and median/tail error gates are shown; a failed model cannot replace the current result.",
    ...
    "zh-TW": {
      ...
      methodAria: "屬性效率計算方式", methodTitle: "如何計算？", methodDataTitle: "1. 收集實戰樣本", methodDataDescription: "僅使用在支援副本中，同時確認最終屬性、職業、技能、攻擊方向與首領狀態的傷害。", methodModelTitle: "2. 建立職業模型", methodModelDescription: "交叉驗證不同使用者的樣本，並限制單一安裝來源的上傳權重。", methodCompareTitle: "3. 重新計算提升前後", methodCompareDescription: "在相同條件下將各屬性調整 +1、+10 或 +1%p，比較預期首領傷害的變化。", methodValidationTitle: "4. 通過驗證後公開", methodValidationDescription: "僅顯示通過不同使用者數與中位／尾端誤差門檻的職業結果；失敗模型不會取代目前結果。",
  ...
  function renderResults(result, mode = "personal") {
	...
	const effects = Array.isArray(result.effects)
		? result.effects.filter(effect => !HIDDEN_EFFECTS.has(effect.key))
		: [];
	const groups = [
	...
	if (directionalItems.length) {
		...
		grid.append(...directionalItems.map(effect => {
			...
			condition.textContent = Number.isFinite(share)
				? `${conditionText} · ${t("observedDamageShare", { share: displayNumber(share) })}`
				: conditionText;
			card.append(name, gain, condition);
			return card;
		}));
