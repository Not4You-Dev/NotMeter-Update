  ...
  function renderResults(result, mode = "personal") {
	...
	if (directionalItems.length) {
		...
		grid.append(...directionalItems.map(effect => {
			...
			const condition = document.createElement("small"); condition.textContent = t(
				effect.key === "frontDamageAmplification" ? "frontCondition" : "backCondition");
